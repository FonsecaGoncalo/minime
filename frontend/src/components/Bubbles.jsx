...
const AssistantBubbleBase = ({ text, loading, finished }) => (
    ...
    >
        <div
            className="bg-white/10 backdrop-blur-md border border-white/20 px-4 py-3 rounded-2xl rounded-bl-sm text-sm text-white shadow-lg">
            {loading && !text ? (
...
const UserBubbleBase = ({ text }) => (
    ...
    >
        <div
            className="inline-block bg-indigo-500/20 backdrop-blur-md border border-indigo-400/30 px-4 py-3 rounded-2xl rounded-br-sm text-sm text-white shadow-lg">
            {text}
        </div>
    </motion.div>
);
