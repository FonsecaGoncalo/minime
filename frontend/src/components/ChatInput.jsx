...
export default function ChatInput({landing, value, setValue, onSend, disabled, rightExtras, variant}) {
    ...
    const shell = isCli
        ? `h-14 bg-transparent flex items-center border-t border-borderCosmos px-3`
        : `
    ...
    return (
        <div className="sticky bottom-[calc(env(safe-area-inset-bottom)+0.5rem)] w-full z-10 mb-2">
            <div className="max-w-screen-md mx-auto px-3 sm:px-0">
                <div className={shell}>
                    {isCli && (
                        <span className="terminal-prompt text-sm mr-2 select-none">visitor@gfonseca:~$</span>
                    )}
                    <input
                        className={`flex-1 bg-transparent ${isCli ? 'text-sm' : 'text-base'} text-ink caret-brand placeholder:text-muted outline-none ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                        placeholder={isCli ? "type a command…" : "Ask me anything"}
                        value={value}
                        onChange={(e) => setValue(e.target.value)}
                        onKeyDown={(e) => !disabled && e.key === 'Enter' && onSend()}
                        autoComplete="off"
